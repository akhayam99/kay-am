...
const seedThrough142 = async (): Promise<Database> => {
  const db = makeTestDatabase();
  await migrate(
    db,
    migrations.filter((migration) => migration.version <= 142),
  );
  await db.execute(
...
describe('m143 session event pr discovered', () => {
  it('accepts the pr_discovered kind next to the existing kinds', async () => {
    const db = await seedThrough142();
