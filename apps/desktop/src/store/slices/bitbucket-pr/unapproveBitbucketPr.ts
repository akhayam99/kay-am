import { bitbucketUnapprovePullRequest } from '../../../features/integrations/bitbucket/client';
import { runBitbucketPrWrite } from './runBitbucketPrWrite';
import type { BitbucketPrWriteParams, GetFn, SetFn } from './types';

export const unapproveBitbucketPr = (set: SetFn, get: GetFn) => {
  return async ({ sessionId, mountId, repo, pullRequestId }: BitbucketPrWriteParams) => {
    await runBitbucketPrWrite({
      ...
      pullRequestId,
      write: async () => {
        await bitbucketUnapprovePullRequest({ ...repo, pullRequestId });
      },
    });
  };
};
